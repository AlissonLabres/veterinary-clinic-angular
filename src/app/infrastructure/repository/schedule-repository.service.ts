...
export class ScheduleRepositoryService implements ScheduleRepositoryInterface {
  ...
  cancelSchedule(id: number): Observable<void> {
    ...
    return this.httpClient.post<void>(
      `${environment.api}/schedule/cancel`,
      input
    );
  }
  ...
  createSchedule(
    ...
    return this.httpClient.post<ScheduleEntity>(
      `${environment.api}/schedule/appointment`,
      input
    );
  }

  getScheduleByUser(id: string): Observable<ScheduleEntity[]> {
    return this.httpClient.get<ScheduleEntity[]>(
      `${environment.api}/schedule/${id}`
    );
  }
}
